...
class DQNExperiment(BaseExperiment):
    ...
    def reset(self):
        ...
        # Sensor stack
        self.prev_rgb_0 = None
        self.prev_rgb_1 = None
        self.prev_rgb_2 = None
    ...
    def get_observation_space(self):
        ...
        image_space = Box(
            ...
            shape=(
                84,
                84,
                num_of_channels * self.frame_stack * 2,
            ),
            dtype=np.float32,
        )
        print(image_space.shape)  # (300,300,12)
        return image_space

    def get_actions(self):
        return {
            0: [0.5, 0.75, 0.0, False, False],  # Straight
            1: [0.5, 0.50, 0.0, False, False],  # Right
            2: [0.5, 0.25, 0.0, False, False],  # Right
            3: [0.5, 0.0, 0.0, False, False],  # Right
            4: [0.5, -0.25, 0.0, False, False],  # Left
            5: [0.5, -0.50, 0.0, False, False],  # Left
            6: [0.5, -0.75, 0.0, False, False],  # Left
        }
    ...
    def get_observation(self, sensor_data):
        ...
        """
        image = post_process_image(sensor_data['birdview'][1], normalized=True, grayscale=True, rgb=False)
        rgb = post_process_image(sensor_data['rgb'][1], normalized=True, grayscale=True, rgb=True)
        cv2.waitKey(1)
        ...
        if self.prev_rgb_0 is None:
            self.prev_rgb_0 = image
            self.prev_rgb_1 = self.prev_rgb_0
            self.prev_rgb_2 = self.prev_rgb_1
        ...
        images = np.concatenate([images, rgbs], axis=2)
        return images, {}
    ...
    def get_done_status(self, observation, core):
        ...
        if self.get_speed(hero) > 1.0:
            ...
            self.time_idle += 1
        self.time_episode += 1
        self.done_time_episode = 10000 < self.time_episode
